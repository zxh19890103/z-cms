...
begin
	...
	begin
         ...
         set @baseCrumbs=(Select Crumbs From [Nt_CourseCategory] where id=@parent)
	end
	...
	insert into [Nt_CourseCategory] (name,parent,depth,crumbs,
clickrate,Display,DisplayOrder,Language_Id,FitPeople,EduTeachers,CourseType,EduAim)values(@name,@parent,
@currentDepth,'',0,@display,@displayorder,{lang},'','','','')
	set @currentID=(select @@IDENTITY)
	update [Nt_CourseCategory] 
