...
namespace Nt.DAL
{
    public class CommonFactory2
    {
            ...
            where M : BaseViewModel, new()
        {
            ...
            FetchListBySql<M>(sql, data);

            return data;
        }

        static void FetchListBySql<M>(string sql, List<M> data)
            where M : BaseViewModel, new()
        {
            ...
            where M : BaseViewModel, new()
        {
            ...
            if (pageindex == 1)
            {
                sql = string.Format("Select Top {0} * From [{1}] Order by {2} ", pagesize, table, orderby);
            }
            else
            {
                string subSelect = string.Format("Select Top {0} ID From [{1}] Order By {2}",
                    (pageindex - 1) * pagesize, table, orderby);
                sql = string.Format("Select Top {0} * From [{1}] Where ID Not in ({2}) Order by {3}",
            ...
            where M : BaseViewModel, new()
        {
            ...
            if (pageindex == 1)
            {
                sql = string.Format("Select Top {0} * From [{1}] Where {2} ", pagesize, table, filter);
            }
            else
            {
                string subSelect = string.Format("Select Top {0} ID From [{1}] Where {2}",
                    (pageindex - 1) * pagesize, table, filter);
                sql = string.Format("Select Top {0} * From [{1}] Where {2} And ID Not in ({3})",
            ...
            where M : BaseViewModel, new()
        {
            ...
            if (pageindex == 1)
            {
                sql = string.Format("Select Top {0} * From [{1}] ", pagesize, table);
            }
            else
            {
                string subSelect = string.Format("Select Top {0} ID From [{1}]",
                    (pageindex - 1) * pagesize, table);
                sql = string.Format("Select Top {0} * From [{1}] Where ID Not in ({2})",
